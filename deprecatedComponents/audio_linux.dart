...
Stream stateStream = player.playbackStateStream.asBroadcastStream();
...
Future<void> initAudio() async {
  player = SimpleAudio(
    onSkipNext: (a) => skipTo(current.value + 1),
    onSkipPrevious: (a) => skipTo(current.value - 1),
  ...
  player.progressStateStream.listen((position) async {
    if (position.position > 4 && position.duration == position.position && !loading) {
      loading = true;
      await skipTo(
        {
...
Future<void> playItem(s.MediaItem item) async {
  ...
  await player.open(item.extras!['url']);
  unawaited(preload());
}
    ...
    return ValueListenableBuilder<int>(
      ...
      builder: (context, snapIndex, widget) {
        return StreamBuilder(
          stream: stateStream.asBroadcastStream(),
